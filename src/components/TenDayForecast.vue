<template>
  <div class="mb-16">
    ...
    <div id="ten-day-scroller" class="flex flex-no-wrap justify-between ml-2 mb-4 scrolling-touch">
      <div class="text-center" v-for="dayIndex in this.$store.getters.openWeatherForecast.daily" :key="dayIndex.myId">
        ...
        <div class='flex items-end pb-2 space-x-2'>
          <p class='text-md'>{{ dayIndex.temp.max.toFixed() }}<span v-html="degreeSymbol"></span></p>
          <p class="pb-px text-sm text-gray-700">{{ dayIndex.temp.min.toFixed() }}<span v-html="degreeSymbol"></span></p>
        </div>
